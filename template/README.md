...
Démo du lab Absolu, publiée sur **https://lab.agence-absolu.com/{{slug}}/**.
...
base des chemins du nom npm (`{{slug}}`). `BASE_PATH=/ npm run build` pour une
racine de domaine.

## Publication

`.github/workflows/deploy.yml` compile et envoie `dist/` par rsync sur SSH dans
`lab.agence-absolu.com/lab-projects/{{slug}}/` à chaque push sur `main` — ou à
la demande, onglet Actions. Le hub sert ce dossier sur `/{{slug}}/` sans
